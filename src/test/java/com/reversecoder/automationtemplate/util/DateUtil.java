...
public class DateUtil {
    ...
    public DateUtil beginOfDay() {
        return new DateUtil(new DateTime(date.getYear(), date.getMonthOfYear(), date.getDayOfMonth(), 0, 0));
    }
    ...
    public DateUtil beginOfMonth() {
        return new DateUtil(new DateTime(date.getYear(), date.getMonthOfYear(), 1, 0, 0));
    }
    ...
    public boolean isBetweenInTime(DateUtil date1, DateUtil date2) {
        if (date.getMillisOfDay() >= date1.date.getMillisOfDay() && date.getMillisOfDay() < date2.date.getMillisOfDay())
            return true;
        return false;
    }

    public boolean isBetweenDay(DateUtil date1, DateUtil date2) {
        if (!date.isBefore(date1.date) && !date.isAfter(date2.date))
            return true;
        return false;
    }
