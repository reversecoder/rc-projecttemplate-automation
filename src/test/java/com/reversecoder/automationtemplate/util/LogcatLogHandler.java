...
public class LogcatLogHandler {
    public LogcatLog takeLog(LogcatLog log, AppiumDriver driver) {
        try {
            ...
            File logFile = new File(getLogFileDirectory() + "/" + log.getLogFile() + ".txt");
            if (logFile.exists()) {
                logFile.delete();
                logFile = new File(getLogFileDirectory() + "/" + log.getLogFile() + ".txt");
            }
            PrintWriter log_file_writer = new PrintWriter(logFile);
            for (LogEntry entry : logEntries) {
                log_file_writer
                        .println("[" + new DateUtil(entry.getTimestamp()).toDateTimeStr() + "]: " + entry.getMessage());
            }
