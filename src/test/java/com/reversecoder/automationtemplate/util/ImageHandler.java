...
public class ImageHandler {
    ...
    public class Color {
        ...
        public Boolean isWithinRed(int r, Color deviation) {
            if (this.red >= (r - deviation.red) && this.red <= (r + deviation.red))
                return true;
            else
                return false;
        }
        ...
        public Boolean isWithinGreen(int g, Color deviation) {
            if (this.green >= (g - deviation.green) && this.green <= (g + deviation.green))
                return true;
            else
                return false;
        }
        ...
        public Boolean isWithinBlue(int b, Color deviation) {
            if (this.blue >= (b - deviation.blue) && this.blue <= (b + deviation.blue))
                return true;
            else
                return false;
        }
    ...
    public Boolean isGreenCircle(ScreenShot screenshot, int x, int y) throws IOException {
        int rgb = getRGBValueFromImagePixel(screenshot, x, y);
        return green.isWithinRed(getRedValue(rgb), deviation) && green.isWithinGreen(getGreenValue(rgb), deviation)
                && green.isWithinBlue(getBlueValue(rgb), deviation);
    }

    public Boolean isOrangeCircle(ScreenShot screenshot, int x, int y) throws IOException {
        int rgb = getRGBValueFromImagePixel(screenshot, x, y);
        return orange.isWithinRed(getRedValue(rgb), deviation) && orange.isWithinGreen(getGreenValue(rgb), deviation)
                && orange.isWithinBlue(getBlueValue(rgb), deviation);
    }

    public Boolean isBluePixel(int rgb) throws IOException {
        return blue.isWithinRed(getRedValue(rgb), deviation) && blue.isWithinGreen(getGreenValue(rgb), deviation)
                && blue.isWithinBlue(getBlueValue(rgb), deviation);
    }

    public Boolean isBluePixel(Color color) throws IOException {
        return ((color.blue - color.red) > 50) ? true : false;
    }

    public Boolean isWhitePoint(ScreenShot screenshot, int x, int y) throws IOException {
        return whiteRGBValue == getRGBValueFromImagePixel(screenshot, x, y);
    }

    public int getRGBValueFromImagePixel(ScreenShot screenshot, int x, int y) throws IOException {
