...
public class ScreenshotHandler {
	...
	public ScreenShot takeScreenShot(ScreenShot screenshot,
			AppiumDriver driver, WebElement paramWebElement) {
		try {
			if(driver == null) return screenshot;
			TakesScreenshot temp = (TakesScreenshot) driver;
			File file = temp.getScreenshotAs(OutputType.FILE);
			if (file.exists()) {
				File saveFile = new File(getScreenShotsDirectory() + "/"
						+ screenshot.getScreenShotFile() + ".png");
				if (saveFile.exists()) {
					saveFile.delete();
				}
	...
	private void writeElementImage(File srcFile, WebElement paramWebElement,
			File dstFile) throws Exception {
		SeekableStream seekableStream = new FileSeekableStream(srcFile);
		PNGDecodeParam pngParams = new PNGDecodeParam();
		ImageDecoder dec = ImageCodec.createImageDecoder("png", seekableStream,
				pngParams);
		...
		BufferedImage outputBufferedImage = inputBufferedImage.getSubimage(
				localPoint.getX(), localPoint.getY(), paramWidth, paramHeight);
