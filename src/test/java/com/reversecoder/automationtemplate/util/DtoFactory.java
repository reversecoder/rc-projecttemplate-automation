        ...
        try {
            ...
            for (Field field : fields) {
                ...
                if (Dto.class.isAssignableFrom(fieldType)) {
                    Class<?> c = Class.forName(field.getType().getCanonicalName());
                    Dto screenDto = new CSVHandler()
                            .getTestData(field.getName().toUpperCase(), (Dto) c.newInstance(), ResourceType.SCREEN_DATA)
                            .get(0);
    ...
    public static <T> void initPageDto(T page) {
        try {
            ...
            for (Field field : fields) {
                ...
                if (Dto.class.isAssignableFrom(fieldType)) {
                    Class<?> c = Class.forName(field.getType().getCanonicalName());
                    Dto pageDto = new CSVHandler()
                            .getTestData(field.getName().toUpperCase(), (Dto) c.newInstance(), ResourceType.PAGE_DATA)
                            .get(0);
